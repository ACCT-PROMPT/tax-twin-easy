...
// --- SUPABASE AUTH + ENTITLEMENT GATE ---
// anon/public key — safe to embed client-side, real access control is Row Level Security
// in the database, not this key. Never put the service_role key here.
const SUPABASE_URL = "https://cxyehagldwanxfjfwopj.supabase.co";
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN4eWVoYWdsZHdhbnhmamZ3b3BqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODYzMzUyNDUsImV4cCI6MjEwMTkxMTI0NX0.TXouM5zmc2_ifTlPz90RcTYpJez9rGuyP_7MLPJwEHM";
const STORE_URL = "http://localhost:3003";
const TOOL_SLUG = "tax-twin-easy";
...
function AuthGate({ children }: { children: React.ReactNode }) {
  const [status, setStatus] = useState<"loading" | "signed_out" | "denied" | "session_full" | "allowed">("loading");
  ...
  async function checkEntitlementAndClaimSession(): Promise<void> {
    const { data: toolRows } = await supabase.from("tools").select("id").eq("slug", TOOL_SLUG).single();
    if (!toolRows) {
      setStatus("denied");
      return;
    }
    const { data: entRows } = await supabase
      ...
      .eq("tool_id", toolRows.id)
      .eq("status", "active");

    if (!entRows || entRows.length === 0) {
      setEntitlement({ allowed: false, expiresAt: null });
      setStatus("denied");
      return;
    }
    setEntitlement({ allowed: true, expiresAt: entRows[0].expires_at });

    // Reuse an existing device session (stored in localStorage) if the heartbeat still succeeds,
    // otherwise claim a new one — mirrors the concurrent-screen limiting the other tools use.
    const deviceKey = `taxtwineasy_device_session_${TOOL_SLUG}`;
    const existingSid = localStorage.getItem(deviceKey);
    if (existingSid) {
      const { data: alive } = await supabase.rpc("heartbeat_session", { p_session_id: existingSid });
      if (alive) {
        setStatus("allowed");
        return;
      }
    ...
    const { data: claim } = await supabase.rpc("claim_session", {
      p_tool_slug: TOOL_SLUG,
      p_device_label: userEmail,
    });
    const row = claim?.[0];
    if (row?.allowed) {
      localStorage.setItem(deviceKey, row.session_id);
      setStatus("allowed");
    } else {
      setSessionInfo({ count: row?.active_count ?? 0, max: row?.max_allowed ?? 0 });
      setStatus("session_full");
    }
  }
  ...
  useEffect(() => {
    if (userEmail) checkEntitlementAndClaimSession();
  ...
  async function handleLogout() {
    const deviceKey = `taxtwineasy_device_session_${TOOL_SLUG}`;
    const sid = localStorage.getItem(deviceKey);
    if (sid) {
  ...
  if (status === "signed_out") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 to-indigo-950 px-4">
        <form
        ...
        >
          <div className="flex items-center gap-2 justify-center mb-2">
            ...
            <h1 className="font-bold text-lg text-slate-900">Tax Twin Easy — เข้าสู่ระบบ</h1>
          </div>
          <p className="text-xs text-slate-500 text-center -mt-2">เข้าสู่ระบบด้วยบัญชีที่ซื้อเครื่องมือนี้จาก Store</p>
          <input
            type="email"
            required
            placeholder="อีเมล"
            ...
            className="border border-slate-300 rounded-lg px-3.5 py-2.5 text-sm outline-none focus:border-indigo-500"
          />
          <input
            type="password"
            required
          ...
          <button
            type="submit"
            disabled={submitting}
            className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-sm py-2.5 rounded-lg transition"
          >
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm text-center flex flex-col gap-4">
          <h1 className="font-bold text-lg text-slate-900">Tax Twin Easy</h1>
          <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3.5 py-3">
            บัญชีนี้ยังไม่มีสิทธิ์ใช้งานเครื่องมือนี้ กรุณาซื้อเครื่องมือนี้ที่ Store ก่อน
          </p>
          <a
            href={STORE_URL}
            className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm py-2.5 rounded-lg transition"
          >
            ไปที่ Store
          </a>
          <button type="button" onClick={handleLogout} className="text-xs text-slate-400 underline">
            ออกจากระบบ
          </button>
        </div>
      </div>
  ...
  if (status === "session_full") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm text-center flex flex-col gap-4">
          <h1 className="font-bold text-lg text-slate-900">Tax Twin Easy</h1>
          <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3.5 py-3">
            ถึงจำนวนหน้าจอสูงสุดของแพ็กนี้แล้ว ({sessionInfo?.count ?? 0}/{sessionInfo?.max ?? 0} จอ)
            กรุณาออกจากระบบในอุปกรณ์อื่นก่อน จึงจะเข้าใช้งานหน้านี้ได้
          </p>
          <button
            type="button"
    ...
    <div className="relative">
      <div className="fixed top-3 right-3 z-50">
        <button
          ...
          className="flex items-center gap-1.5 bg-white/90 backdrop-blur border border-slate-200 shadow-sm text-slate-600 text-xs font-semibold px-3 py-1.5 rounded-full hover:bg-white"
        >
